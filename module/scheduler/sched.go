...
func (s *Scheduler) jobEnqueue(job *Job) error {
	s.Mutex.Lock()
...
func (s *Scheduler) jobDequeue(option *ConsumerOption) *Job {
	s.Mutex.Lock()
	defer s.Mutex.Unlock()
...
func (s *Scheduler) jobDone(option *ConsumerOption, job *Job) {
	s.Mutex.Lock()
	defer s.Mutex.Unlock()
	...
	if job.Count == job.Done {
		...
		job.Save()
		callback, found := s.Callback[job.Type]
		if found {
